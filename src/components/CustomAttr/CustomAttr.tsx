...
function CustomAttr({ position, attrs, parentId }) {
        ...
        >
          <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
            ...
            <div style={{ minWidth: '50px', display: !disabled ? 'flex' : 'none', alignItems: 'center' }}>
              <IconButton
                aria-label="add"
                onClick={(event) => { event.stopPropagation(); toggleStateModal(true) }}
        ...
        <ExpansionPanelDetails >
          <div>
            {Object.keys(attrs).map((attr, i) => (
              <Typography key={i}>{`${attr} : ${attrs[attr]}`}</Typography>
            ))}
          </div>
          <CustomModal isOpen={isOpen} toggleStateModal={toggleStateModal} parentId={parentId} />
        </ExpansionPanelDetails>
      </ExpansionPanel>
    </div>
  );
